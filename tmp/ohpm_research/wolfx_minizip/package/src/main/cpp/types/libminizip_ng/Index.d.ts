...
export class MinizipNative{
  ...
  // 流式解压相关方法
  OpenEntryForStream : (entryName : string, password : string) => number;
  ReadEntryStream : (buffer: ArrayBuffer, size: number) => number;
  CloseEntryStream : () => number;
  // 获取ZIP文件总大小
  GetTotalUncompressedSize : () => number;
  // 获取特定文件的大小
  GetEntryUncompressedSize : (entryName: string) => number;
}
...
export class MinizipCompressNative{
  ...
  // 流式压缩相关方法
  CreateEntryForStream : (entryName : string, password : string) => number;
  WriteEntryStream : (buffer: ArrayBuffer, size: number) => number;
  CloseEntryStream : () => number;
}