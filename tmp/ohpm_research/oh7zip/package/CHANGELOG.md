## v1.0.3

- add the causes of error code 3 and error code 4 returned during the compression process
- add verification demo

## v1.0.2

- perf: optimize library with O3 optimization level

## v1.0.2-rc.0

modify docs

- add new fileds for oh-package.json5

## v1.0.1

fix bug

- Fixed garbled text issue when unzipping macOS-compressed zip files on Openharmony.

Implemented Feature

- Supports compressing multiple paths and excluding specific files when compressing.

## v1.0.0

Implemented Feature
- 1.File compression, supports 7z and zip formats.
- 2.File decompression, supports rar, 7z, zip, tar, gz formats