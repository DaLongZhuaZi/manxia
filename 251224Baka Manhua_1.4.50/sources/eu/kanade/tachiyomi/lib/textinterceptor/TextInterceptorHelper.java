    ...
    public final int countLines(String html) {
        ...
        try {
            if (Build.VERSION.SDK_INT >= 24) {
                string = Html.fromHtml(html, 0).toString();
            } else {
                string = Html.fromHtml(html).toString();
            }
            ...
            if (Build.VERSION.SDK_INT >= 23) {
                staticLayout = StaticLayout.Builder.obtain(string, 0, string.length(), textPaint, 900).setAlignment(Layout.Alignment.ALIGN_NORMAL).setLineSpacing(1.0f, 1.1f).setIncludePad(true).build();
            } else {
                staticLayout = new StaticLayout(string, textPaint, 900, Layout.Alignment.ALIGN_NORMAL, 1.1f, 1.0f, true);
            }
