...
param(
  ...
  [string]$OutputPath = 'tools/legado-compat/evidence/v2-jsoup-element-select-context-self-current-head-audit-20260810.json'
)
...
function Assert-Audit { param([bool]$Condition, [string]$Message); if (-not $Condition) { throw "243 Element.select context-self current-head audit failed: $Message" }; $script:assertions++ }
...
Assert-Audit ([string]$fixture.contract -eq 'legado_jsoup_element_select_context_self' -and @($fixture.cases).Count -eq 4 -and @($fixture.representativeSourceSet.sourceOrdinals).Count -eq 3) 'context-self fixture drifted.'
...
Assert-Audit ($element.Contains('  select(selector: string): HTMLElement[] {') -and $element.Contains('return this.selectAll(selector, true);') -and $element.Contains('private getSelectionDocumentOrder(includeSelf: boolean): HTMLElement[]') -and $element.Contains("return this.tagName === 'root' && this.parentNode === null;") -and $element.Contains('private isWithinSelectionScope(element: HTMLElement, includeSelf: boolean): boolean')) 'current DOM matcher does not retain explicit Element.select context semantics.'
Assert-Audit ($bridge.Contains('results = context.select(`.${className}`);') -and $bridge.Contains('const elem = context.select(`#${idName}`)[0];') -and $bridge.Contains('results = context.select(tagName);') -and $bridge.Contains('results = context.select(selector);')) 'current bridge does not retain Jsoup-style chain context routing.'
...
$result = [pscustomobject][ordered]@{
  ...
  verificationPolicy = 'r3_243_element_select_context_self_current_head_static_only;runtime_build_device_and_legado_diff_deferred_to_R4'
}
