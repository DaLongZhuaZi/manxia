...
param(
  ...
  [string]$OutputPath = 'tools/legado-compat/evidence/v2-arkweb-jsoup-index-middle-compound-current-head-audit-20260810.json'
)
...
function Assert-Audit {
  ...
  if (-not $Condition) { throw "ArkWeb 243 index middle-compound current-head audit failed: $Message" }
  $script:assertions++
}
...
Assert-Audit ([string]$fixture.contract -eq 'arkweb_legado_jsoup_index_pseudo_middle_compound_context' -and @($fixture.cases).Count -eq 4 -and [int]$fixture.sourcePackageScan.middleCompoundRuleStringCount -eq 0) 'middle-compound fixture or package scan drifted.'
...
Assert-Audit ($runtime.Contains("var siblingIndex = legadoElementSiblingIndex(predicateNode);") -and -not $runtime.Contains("var siblingIndex = legadoElementSiblingIndex(node);")) 'current head does not retain owning predicateNode semantics for eq/lt.'
...
$result = [pscustomobject][ordered]@{
  ...
  verificationPolicy = 'r3_243_arkweb_index_middle_compound_current_head_static_only;runtime_build_device_and_legado_diff_deferred_to_R4'
