...
param(
  ...
  [string]$AnalyzerSnapshotPath = 'entry/src/main/ets/Framework/Novel/LegadoRuleAnalyzer.ets.bak_20260810_not_ancestor_context',
  [string]$OutputPath = 'tools/legado-compat/evidence/contract-legado-jsoup-not-selector-ancestor-pre-fix-20260810.json'
)
...
function Assert-Witness { param([bool]$Condition, [string]$Message); if (-not $Condition) { throw "243 :not ancestor failure witness failed: $Message" } }
...
Assert-Witness ($analyzer.Contains('const wrapper = `<legado-not-root>${parentElement}</legado-not-root>`')) 'pre-fix parent-only wrapper is absent.'
Assert-Witness (-not $analyzer.Contains('selectorContextHtml')) 'pre-fix root selector context unexpectedly exists.'
Assert-Witness ($analyzer.Contains('const occurrences = this.mapStringElementOccurrences(contextHtml, [element])')) 'pre-fix candidate occurrence lookup is absent.'
Assert-Witness ($analyzer.Contains('matches.some((match: string): boolean => match === element)') -or $analyzer.Contains('targetRelativeStart')) 'pre-fix candidate projection marker is absent.'
...
$result = [pscustomobject][ordered]@{
  schemaVersion = 1
  kind = 'legado_jsoup_not_selector_ancestor_pre_fix_contract'
  issueId = 'ISSUE-COMPAT-243-JSOUP-STANDARD-CSS-PSEUDO-SELECTORS'
  status = 'failed'
  generatedAt = [DateTimeOffset]::UtcNow.ToString('o')
  classification = 'v2_string_fallback_not_selector_lost_full_ancestor_context_and_occurrence_identity'
  ...
  rootCauseDecision = 'The current :not fallback retains only the immediate parent and resolves every duplicate candidate by its first HTML occurrence; selectors that require a deeper ancestor or a later identical node therefore widen or reject the wrong candidate.'
  staticWitnesses = @(
    [pscustomobject][ordered]@{ path = $AnalyzerSnapshotPath; finding = 'matchesStringNestedSelector builds a synthetic document from parentElement instead of the original selector root, and maps the candidate by first matching HTML.' },
    [pscustomobject][ordered]@{ path = $legadoPath; finding = 'pinned Legado evaluates Element.select against the complete document and preserves node identity.' }
  )
  runtimeActionsPerformed = @()
  semanticMatchAllowed = $false
  verificationPolicy = 'r3_243_not_selector_ancestor_failure_witness_static_only;runtime_regression_build_device_and_legado_diff_deferred'
}
