param(
  ...
  [string]$PostFixContractPath = 'tools/legado-compat/evidence/contract-legado-jsoup-attribute-and-tag-token-post-fix-20260810.json',
  [string]$CurrentHeadAuditPath = 'tools/legado-compat/evidence/v2-jsoup-attribute-and-tag-token-current-head-audit-20260810.json',
  [string]$SourceFixPath = 'tools/legado-compat/evidence/v2-jsoup-attribute-and-tag-token-source-fix-20260810.json'
)
...
$revision = '2026-08-10-actual-docs-source-refactor-jsoup-attribute-and-tag-token-static-closure-r1'
...
function Assert-Gate { param([bool]$Condition, [string]$Detail); if (-not $Condition) { throw "243 attribute/tag token source-fix gate failed: $Detail" } }
...
Assert-Gate ([string]$fixture.issueId -eq $issueId -and @($fixture.cases).Count -eq 3) 'attribute/tag token fixture drifted.'
Assert-Gate ($analyzer.Contains('const scan = this.scanNextHtmlTag(content, cursor);') -and $analyzer.Contains('searchStart = startTagEnd + 1;') -and -not $analyzer.Contains('searchStart = tagStart + 1;') -and $analyzer.Contains("this.extractAttributeValueFromTagContent(tagContent, 'class')") -and $analyzer.Contains("this.extractAttributeValueFromTagContent(tagContent, 'id')") -and -not $analyzer.Contains('this.findClassAttrIndex(') -and -not $analyzer.Contains('this.findIdAttrIndex(')) 'scanner-backed tag and attribute source closure is missing.'
...
$sourceFix = [pscustomobject][ordered]@{
  ...
  extends = 'tools/legado-compat/evidence/v2-jsoup-element-extraction-source-fix-20260810.json'
  ...
  rootCause = [pscustomobject][ordered]@{
    category = '规则解析或编译'
    before = 'Tag selection advanced one raw less-than token at a time, while class and id lookup searched raw class=/id= text and reconstructed a tag by scanning backwards. Markup-like and attribute-like text inside quoted values could therefore create false matches.'
    after = 'Tag, class and id lookup all advance through the shared quote-aware HtmlTagScan and resume after the complete start tag. Attribute extraction now parses the opening tag and consumes quoted values atomically before comparing the requested attribute name.'
    legadoReference = 'Pinned Legado AnalyzeByJSoup delegates tag, class and id selection to Jsoup Element nodes and parsed attributes; quoted attribute data is not reparsed as structure.'
  }
  consumerMatrix = [pscustomobject][ordered]@{
    tagSelector = $analyzerPath
    classSelector = $analyzerPath
    idSelector = $analyzerPath
    compoundSelector = $analyzerPath
    imageAttributeResolution = $analyzerPath
    legado = 'legado/app/src/main/java/io/legado/app/model/analyzeRule/AnalyzeByJSoup.kt'
  }
  runtimeActionsPerformed = @()
  semanticMatchAllowed = $false
  verificationPolicy = 'r3_243_attribute_and_tag_token_source_fix_static_only;R4_runtime_build_device_and_legado_diff_deferred'
...
Set-PropertyValue $objective 'continuationMode' 'R3_ISSUE_243_ATTRIBUTE_AND_TAG_TOKEN_STATIC_CLOSED_WAIT_R4'
Set-PropertyValue $objective.authority 'activeIssueId' $issueId
Set-PropertyValue $objective.authority 'activeIssueSelection' 'full-source-validation-state.json remains the only queue; 243 remains the sole active source-closure issue. Tag, class and id lookup now share quote-aware tag and parsed-attribute boundaries; semanticMatchAllowed remains false and R4 is deferred.'
Set-PropertyValue $objective.objective 'activeIssueRule' '243 standard CSS/XPath traversal must never reparse quoted attribute data as HTML nodes or attributes; tag, class and id selection must consume only scanned element boundaries and parsed attributes.'
Set-PropertyValue $objective 'nextAction' '243 attribute/tag token static closure is registered; continue one-root-cause source audit while R4 remains deferred.'
$plan = @($objective.continuationPlan)
if (-not @($plan | Where-Object { [string]$_.id -eq '243-SP-43' })) {
  $plan += [pscustomobject][ordered]@{ id = '243-SP-43'; status = 'completed'; action = '统一 tag、class、id 选择的结构扫描和属性解析边界，禁止引号属性值中的 <tag>/class=/id= 文本伪造节点或属性。'; evidence = @($FixturePath, $FailureWitnessPath, $PostFixContractPath, $CurrentHeadAuditPath, $SourceFixPath) }
  $plan += [pscustomobject][ordered]@{ id = '243-SP-44'; status = 'deferred'; action = 'R4 执行三个 token-context fixture、影响源集合、458 条 Harness、Legado 差分、构建和真机验证。' }
...
$evidence = @($FixturePath, $FailureWitnessPath, $PostFixContractPath, $CurrentHeadAuditPath, $SourceFixPath, $analyzerPath, $backupPath, 'tools/legado-compat/Test-LegadoJsoupAttributeAndTagTokenFailureWitness.ps1', 'tools/legado-compat/Test-LegadoJsoupAttributeAndTagTokenPostFixContract.ps1', 'tools/legado-compat/Test-LegadoJsoupAttributeAndTagTokenCurrentHeadAudit.ps1', 'tools/legado-compat/Register-LegadoJsoupAttributeAndTagTokenSourceFix.ps1', 'legado/app/src/main/java/io/legado/app/model/analyzeRule/AnalyzeByJSoup.kt')
$summary = '243 tag, class and id lookup now share quote-aware tag scanning and parsed attribute boundaries; quoted attribute data cannot create false structural matches. Runtime, full Harness, Legado differential, build and device validation remain deferred.'
