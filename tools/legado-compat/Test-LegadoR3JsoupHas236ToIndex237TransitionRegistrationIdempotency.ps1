...
param(
  ...
  [string]$OutputPath = 'tools/legado-compat/evidence/r3-jsoup-has-236-to-index-237-transition-20260809/registration-idempotency.json'
)
...
function Assert-Recovery {
  ...
  if (-not $Condition) { throw "237 registration replay blocked: $Detail" }
  $script:assertions++
...
$registerScript = Join-Path $RepositoryRoot 'tools\legado-compat\Register-LegadoR3JsoupHas236ToIndex237Transition.ps1'
$result = $null
$exitCode = 0
try {
  ...
  $issueBefore = Get-Issue -Issues @($stateBefore.governance.issues) -Id 'ISSUE-COMPAT-237-JSOUP-INDEX-PSEUDO-SELECTORS'
  Assert-Recovery ($null -ne $issueBefore) 'issue_present' '237 governance issue exists.' @('tools/legado-compat/state/full-source-validation-state.json')
  ...
  Assert-Recovery ($registrationExitCode -eq 0) 'registration_exit' ('registration replay exited with ' + $registrationExitCode) @('tools/legado-compat/Register-LegadoR3JsoupHas236ToIndex237Transition.ps1')
  $registrationResult = $registrationOutput | ConvertFrom-Json
  Assert-Recovery ([string]$registrationResult.status -eq 'already_registered' -and [bool]$registrationResult.idempotent -and [string]$registrationResult.issueId -eq 'ISSUE-COMPAT-237-JSOUP-INDEX-PSEUDO-SELECTORS') 'already_registered' 'replay used the post-registration branch for 237.' @('tools/legado-compat/Register-LegadoR3JsoupHas236ToIndex237Transition.ps1')
  Assert-Recovery (-not [bool]$registrationResult.semanticMatchAllowed -and @($registrationResult.runtimeActionsPerformed).Count -eq 0) 'static_only' 'replay made no runtime or semantic-match claim.' @('tools/legado-compat/Register-LegadoR3JsoupHas236ToIndex237Transition.ps1')
  ...
  $issueAfter = Get-Issue -Issues @($stateAfter.governance.issues) -Id 'ISSUE-COMPAT-237-JSOUP-INDEX-PSEUDO-SELECTORS'
  ...
  Assert-Recovery ($null -ne $issueAfter -and [int]$issueAfter.attempts -eq $attemptsBefore) 'attempt_unchanged' '237 attempt count remains unchanged after replay.' @('tools/legado-compat/state/full-source-validation-state.json')
  Assert-Recovery ([string]$stateAfter.governance.activeTaskId -eq 'COMPAT-006' -and [string]$stateAfter.governance.activeIssueId -eq 'ISSUE-COMPAT-237-JSOUP-INDEX-PSEUDO-SELECTORS') 'queue_unchanged' 'active machine queue remains on 237.' @('tools/legado-compat/state/full-source-validation-state.json')
  Assert-Recovery ([string]$objectiveAfter.authority.activeIssueId -eq 'ISSUE-COMPAT-237-JSOUP-INDEX-PSEUDO-SELECTORS' -and [string]$objectiveAfter.executionTarget.currentIssue -eq 'ISSUE-COMPAT-237-JSOUP-INDEX-PSEUDO-SELECTORS' -and @($objectiveAfter.executionTarget.nextIssues) -contains 'ISSUE-COMPAT-238-JAVA-OBJECT-CONTENT-OVERLOAD') 'objective_unchanged' 'objective queue remains on 237 with 238 as next candidate.' @('tools/legado-compat/state/refactor-objective.json')

  $result = [pscustomobject][ordered]@{
    schemaVersion = 1
    kind = 'legado_r3_jsoup_has_236_to_index_237_transition_registration_idempotency'
    status = 'passed'
