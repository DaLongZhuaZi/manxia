...
param(
  ...
  [string]$OutputPath = 'tools/legado-compat/evidence/v2-jsoup-not-selector-current-head-audit-20260809.json'
)
...
function Assert-Audit { param([bool]$Condition, [string]$Message); if (-not $Condition) { throw "243 :not selector current-head audit failed: $Message" }; $script:assertions++ }
...
Assert-Audit ([string]$state.governance.activeIssueId -eq 'ISSUE-COMPAT-243-JSOUP-STANDARD-CSS-PSEUDO-SELECTORS' -and -not [bool]$state.governance.semanticMatchAllowed) '243 queue binding drifted.'
...
Assert-Audit ($analyzer.Contains('matchesStringNestedSelector(element, argument, contextHtml)') -and $analyzer.Contains('findParentStartTag(contextHtml, target.startIndex)')) 'context-aware :not consumer is present.'
Assert-Audit ($analyzer.Contains('matchesStringNestedSelectorWithoutContext') -and $analyzer.Contains('targetRelativeStart')) 'explicit top-level fallback and occurrence projection are present.'
...
$result = [pscustomobject][ordered]@{
  schemaVersion = 1
  kind = 'v2_jsoup_not_selector_current_head_audit'
