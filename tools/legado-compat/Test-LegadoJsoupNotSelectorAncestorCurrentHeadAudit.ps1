...
param(
  ...
  [string]$OutputPath = 'tools/legado-compat/evidence/v2-jsoup-not-selector-ancestor-current-head-audit-20260810.json'
)
...
function Assert-Audit { param([bool]$Condition, [string]$Message); if (-not $Condition) { throw "243 :not ancestor current-head audit failed: $Message" }; $script:assertions++ }
...
Assert-Audit ($analyzer.Contains('selectorContextHtml: string =') -and $analyzer.Contains('matchesStringNestedSelectorAtOccurrence')) 'root-context and occurrence-aware consumers are present.'
Assert-Audit ($analyzer.Contains('const wrapper = `<legado-not-root>${contextHtml}</legado-not-root>`') -and $analyzer.Contains('targetRelativeStart')) 'full-context synthetic projection is present.'
...
$result = [pscustomobject][ordered]@{
  schemaVersion = 1
  kind = 'v2_jsoup_not_selector_ancestor_current_head_audit'
