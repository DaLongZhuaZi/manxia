...
if ([string]::IsNullOrWhiteSpace($OutputPath)) {
  $OutputPath = Join-Path $PSScriptRoot 'evidence\legado-jsonpath-runtime-execution-20260807.json'
}

$node = Get-Command node -ErrorAction Stop
$scriptPath = Join-Path $PSScriptRoot 'Test-LegadoJsonPathRuntimeFixture.mjs'
...
if ($LASTEXITCODE -ne 0) {
  throw "Legado JSONPath Runtime fixture failed with exit code $LASTEXITCODE"
}
$jsonLine = @($output | Where-Object { $_ -match '^\{"status":"passed"' } | Select-Object -Last 1)
if ($jsonLine.Count -ne 1) { throw 'Legado JSONPath Runtime fixture did not emit a machine-readable pass result' }
$fixtureResult = $jsonLine[0] | ConvertFrom-Json
if ([string]$fixtureResult.contract -ne 'legado_jsonpath_runtime_execution') {
  throw 'Unexpected JSONPath Runtime fixture contract identifier'
}
$result = [ordered]@{
  schemaVersion = 1
  ...
  reproduction = 'node tools/legado-compat/Test-LegadoJsonPathRuntimeFixture.mjs'
}
