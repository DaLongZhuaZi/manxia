...
param(
  ...
  [string]$AnalyzerSnapshotPath = 'entry/src/main/ets/Framework/Novel/LegadoRuleAnalyzer.ets.bak_20260809_not_context',
  [string]$OutputPath = 'tools/legado-compat/evidence/contract-legado-jsoup-not-selector-pre-fix-20260809.json'
)
...
function Assert-Witness { param([bool]$Condition, [string]$Message); if (-not $Condition) { throw "243 :not selector failure witness failed: $Message" } }
...
Assert-Witness ([string]$fixture.contract -eq 'legado_jsoup_not_selector_context' -and @($fixture.cases).Count -eq 3) 'not-selector fixture changed.'
Assert-Witness ($analyzer.Contains('private matchesStringNestedSelector(element: string, selector: string): boolean')) 'pre-fix nested selector signature is absent.'
Assert-Witness ($analyzer.Contains('this.matchesStringNestedSelector(element, argument)')) 'pre-fix :not call without context is absent.'
Assert-Witness ($analyzer.Contains('<legado-not-root>${element}</legado-not-root>')) 'pre-fix candidate-only wrapper is absent.'
Assert-Witness ($analyzer.Contains('matches.some((match: string): boolean => match === element)')) 'pre-fix candidate equality projection is absent.'
...
$result = [pscustomobject][ordered]@{
  schemaVersion = 1
  kind = 'legado_jsoup_not_selector_pre_fix_contract'
  issueId = 'ISSUE-COMPAT-243-JSOUP-STANDARD-CSS-PSEUDO-SELECTORS'
  status = 'failed'
  generatedAt = [DateTimeOffset]::UtcNow.ToString('o')
  classification = 'v2_string_fallback_not_selector_lost_ancestor_context'
  ...
  rootCauseDecision = 'Large-document :not evaluation needs the candidate element parent/ancestor context; candidate-only wrapping makes a complex selector such as div span miss and causes the negation to widen results.'
  staticWitnesses = @(
    [pscustomobject][ordered]@{ path = $AnalyzerSnapshotPath; finding = 'matchesStringNestedSelector accepts only element and selector, wraps the candidate alone, and compares result HTML to the candidate.' },
    [pscustomobject][ordered]@{ path = $legadoPath; finding = 'pinned Legado delegates the complete selector to Jsoup Element.select, which evaluates the candidate in document context.' }
  )
  runtimeActionsPerformed = @()
  semanticMatchAllowed = $false
  verificationPolicy = 'r3_243_not_selector_failure_witness_static_only;runtime_regression_build_device_and_legado_diff_deferred'
}
