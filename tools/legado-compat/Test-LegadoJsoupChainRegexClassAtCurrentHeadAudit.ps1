...
param(
  ...
  [string]$ResultPath = 'tools/legado-compat/evidence/v2-jsoup-chain-regex-class-at-current-head-audit-20260810.json'
)
...
function Assert-Audit { param([bool]$Condition, [string]$Message); if (-not $Condition) { throw "243 chain regex-class @ current-head audit failed: $Message" }; $script:assertions++ }
...
Assert-Audit ($methodStart -ge 0 -and $splitStart -gt $methodStart -and $splitEnd -gt $splitStart) 'chain splitter boundary is missing.'
$splitBody = $analyzer.Substring($splitStart, $splitEnd - $splitStart)
Assert-Audit ($splitBody.Contains('let parenthesisDepth = 0') -and $splitBody.Contains('let bracketDepth = 0') -and $splitBody.Contains("if (c === '@' && parenthesisDepth === 0 && bracketDepth === 0)")) 'top-level stateful splitter is not present.'
Assert-Audit (-not $splitBody.Contains("this.skipBalanced('[', ']')") -and -not $splitBody.Contains("this.skipBalanced('(', ')')")) 'legacy same-delimiter splitter is still active.'
...
$result = [pscustomobject][ordered]@{
  ...
  verificationPolicy = 'r3_243_chain_regex_class_at_current_head_static_only;runtime_build_device_and_legado_diff_deferred_to_R4'
