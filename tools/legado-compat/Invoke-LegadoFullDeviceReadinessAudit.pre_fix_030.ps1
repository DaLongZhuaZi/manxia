...
function Invoke-HypiumApplicationLifecycle {
  ...
  if ($result.classification -ne 'success') {
    throw "HYPIUM_LIFECYCLE_FAILED mode=$Mode classification=$($result.classification);exitCode=$($result.exitCode)"
  }
  $resultPath = Join-Path $OutputDirectory 'result.json'
  if (-not (Test-Path -LiteralPath $resultPath)) {
    throw "HYPIUM_LIFECYCLE_EVIDENCE_MISSING mode=$Mode"
  }
  $evidence = Get-Content -LiteralPath $resultPath -Raw -Encoding utf8 | ConvertFrom-Json -Depth 30
  if ([string]$evidence.status -ne 'passed' -or -not [bool]$evidence.driver_closed) {
    throw "HYPIUM_LIFECYCLE_UNCONFIRMED mode=$Mode status=$([string]$evidence.status);driverClosed=$([bool]$evidence.driver_closed)"
  }
  return $resultPath
}
