...
param(
  ...
  [string]$OutputPath = 'tools/legado-compat/evidence/v2-jsoup-sibling-combinator-current-head-audit-20260810.json'
)
...
function Assert-Audit { param([bool]$Condition, [string]$Message); if (-not $Condition) { throw "243 sibling combinator current-head audit failed: $Message" }; $script:assertions++ }
...
Assert-Audit ($analyzer.Contains('splitTopLevelCssSiblingCombinatorSelector') -and $analyzer.Contains('matchesStringCombinatorRelation')) 'sibling combinator consumers are present.'
Assert-Audit ($analyzer.Contains('combinator === ''+''') -and $analyzer.Contains('combinator === ''~''')) 'both sibling operators remain explicit.'
...
$result = [pscustomobject][ordered]@{
  schemaVersion = 1
  kind = 'v2_jsoup_sibling_combinator_current_head_audit'
