...
param(
  ...
  [string]$OutputPath = 'tools/legado-compat/evidence/v2-arkweb-jsoup-compound-pseudo-current-head-audit-20260810.json'
)
...
function Assert-Audit { param([bool]$Condition, [string]$Message); if (-not $Condition) { throw "ArkWeb 243 compound pseudo current-head audit failed: $Message" }; $script:assertions++ }
...
Assert-Audit ([string]$fixture.contract -eq 'arkweb_legado_jsoup_compound_pseudo_context' -and @($fixture.cases).Count -eq 4) 'compound pseudo fixture drifted.'
...
Assert-Audit ($runtime.Contains('var legadoSelectorCompoundBefore = function') -and $runtime.Contains("pseudo.targetScope === 'compound'") -and $runtime.Contains('predicateNode = targetFound')) 'current head lacks the owning-compound dispatch.'
...
$result = [pscustomobject][ordered]@{
  ...
  verificationPolicy = 'r3_243_arkweb_compound_pseudo_current_head_static_only;runtime_build_device_and_legado_diff_deferred_to_R4'
}
