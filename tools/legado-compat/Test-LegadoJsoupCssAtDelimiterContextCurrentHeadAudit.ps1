...
param(
  ...
  [string]$ResultPath = 'tools/legado-compat/evidence/v2-jsoup-css-at-delimiter-context-current-head-audit-20260810.json'
)
...
function Assert-Audit { param([bool]$Condition, [string]$Id, [string]$Detail, [string[]]$Evidence = @()); if (-not $Condition) { throw "243 CSS @ delimiter current-head audit failed: $Detail" }; $script:assertions++; [void]$script:checks.Add([pscustomobject][ordered]@{ id = $Id; status = 'passed'; detail = $Detail; evidencePaths = @($Evidence) }) }
...
Assert-Audit ([string]$fixture.issueId -eq $issueId -and @($fixture.cases).Count -eq 3 -and [int]$fixture.largeDocumentThreshold -eq 50000) 'fixture' 'three large-document @ delimiter cases remain bound to 243.' @($FixturePath)
Assert-Audit ($indexStart -ge 0 -and $chainStart -gt $indexStart -and $splitStart -gt $chainStart -and $parseCssStart -gt $splitStart) 'source_boundaries' 'stateful @ splitter boundaries are stable.' @($analyzerPath)
...
Assert-Audit ($indexBody.Contains('this.splitSelectorWithJs(selector)') -and -not $indexBody.Contains("selector.split('@')")) 'index_path' 'legacy-index preflight no longer uses raw @ splitting.' @($analyzerPath)
Assert-Audit ($chainBody.Contains('this.splitSelectorWithJs(selector)') -and -not $chainBody.Contains("selector.split('@')")) 'fallback_path' 'large-document CSS chain no longer uses raw @ splitting.' @($analyzerPath)
Assert-Audit ($splitBody.Contains('parenthesisDepth') -and $splitBody.Contains('bracketDepth') -and $splitBody.Contains('quote') -and $splitBody.Contains('escaped') -and $splitBody.Contains('inJs')) 'scanner_state' 'current head tracks every selector-local context required by Legado chain semantics.' @($analyzerPath)
Assert-Audit ($splitBody.Contains("character === '@' && parenthesisDepth === 0 && bracketDepth === 0")) 'top_level_guard' 'current head emits a chain boundary only for top-level @.' @($analyzerPath)
Assert-Audit ($legado.Contains("lastIndexOf('@')") -and $legado.Contains('temp.select(ruleStr)') -and $legadoRuleAnalyzer.Contains('chompRuleBalanced') -and $legadoRuleAnalyzer.Contains('splitRule')) 'legado_parity_reference' 'pinned Legado reference preserves nested contexts and delegates selector evaluation to Jsoup.' @('legado/app/src/main/java/io/legado/app/model/analyzeRule/AnalyzeByJSoup.kt', 'legado/app/src/main/java/io/legado/app/model/analyzeRule/RuleAnalyzer.kt')
...
$result = [pscustomobject][ordered]@{
  ...
  verificationPolicy = 'r3_243_css_at_delimiter_context_current_head_static_only;runtime_build_device_and_legado_diff_deferred'
