param(
  ...
  [string]$ResultPath = 'tools/legado-compat/evidence/v2-jsoup-compound-selector-comment-current-head-audit-20260810.json'
)
...
function Assert-Audit { param([bool]$Condition, [string]$Id, [string]$Detail, [string[]]$Evidence = @()); if (-not $Condition) { throw "243 compound selector comment current-head audit failed: $Detail" }; $script:assertions++; [void]$script:checks.Add([pscustomobject][ordered]@{ id = $Id; status = 'passed'; detail = $Detail; evidencePaths = @($Evidence) }) }
...
Assert-Audit ([string]$fixture.issueId -eq $issueId -and @($fixture.cases).Count -eq 1) 'fixture' 'the compound comment case remains bound to 243.' @($FixturePath)
...
Assert-Audit ($analyzer.Contains('const scan = this.scanNextHtmlTag(html, searchStart);') -and $analyzer.Contains('searchStart = scan.endIndex + 1;') -and $analyzer.Contains('const currentTagName = scan.tagName;')) 'scanner-consumer' 'compound selector discovery is scanner-backed.' @($analyzerPath)
Assert-Audit (-not $analyzer.Contains("const tagStartIndex = html.indexOf('<', searchStart);") -and -not $analyzer.Contains('searchStart = tagStartIndex + 1;')) 'raw-path-closure' 'the quote-blind compound selector loop is absent.' @($analyzerPath)
...
$result = [pscustomobject][ordered]@{
  ...
  verificationPolicy = 'r3_243_compound_selector_comment_current_head_static_only;runtime_build_device_and_legado_diff_deferred_to_R4'
  nextGate = 'Register the scanner-backed compound selector source fix and keep 243 verifying until R4 runtime and Legado differential gates.'
}
