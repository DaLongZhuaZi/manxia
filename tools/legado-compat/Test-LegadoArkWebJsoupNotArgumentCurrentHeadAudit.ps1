...
param(
  ...
  [string]$OutputPath = 'tools/legado-compat/evidence/v2-arkweb-jsoup-not-argument-current-head-audit-20260810.json'
)
...
function Assert-Audit { param([bool]$Condition, [string]$Message); if (-not $Condition) { throw "ArkWeb 243 :not argument current-head audit failed: $Message" }; $script:assertions++ }
...
Assert-Audit ([string]$fixture.contract -eq 'arkweb_legado_jsoup_not_argument_context' -and @($fixture.cases).Count -eq 3) 'not-argument fixture drifted.'
...
Assert-Audit ($runtime.Contains('var legadoMatchesJsoupSelectorInContext = function') -and $runtime.Contains('legadoSelectWithJsoupRegex(selectionRoot, selector, documentRoot)')) 'current head lacks contextual :not argument projection.'
...
$result = [pscustomobject][ordered]@{
  ...
  verificationPolicy = 'r3_243_arkweb_not_argument_current_head_static_only;runtime_build_device_and_legado_diff_deferred_to_R4'
}
