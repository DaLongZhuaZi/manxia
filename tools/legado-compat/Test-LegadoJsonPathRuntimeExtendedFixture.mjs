...
const payload = {
  code: [
    "var alternative = java.getString('$.missing||$.data[0].title', result);",
    "var recursiveNames = java.getStringList('$..name', result);",
    "var nestedRecursiveUrls = java.getStringList('$.nested..url', result);",
    "var alternatives = java.getStringList('$.rows[*].missing||$.rows[*].url', result);",
    "var regexReplacement = java.getString('$.data[0].title##alpha##ALPHA', result);",
    "var sliceIds = java.getStringList('$.items[:2].id', result);",
    "JSON.stringify({ alternative: alternative, recursiveNames: recursiveNames, nestedRecursiveUrls: nestedRecursiveUrls, alternatives: alternatives, regexReplacement: regexReplacement, sliceIds: sliceIds });"
  ].join('\n'),
  context: {
    result: bodyText,
    ...
    source: {
      ...
      bookSourceName: 'jsonpath-runtime-extended-fixture',
      bookSourceType: '0'
    }
  },
  state: {
...
const actual = JSON.parse(envelope.result);
const expected = fixture.expectedOriginal;
if (JSON.stringify(actual) !== JSON.stringify(expected)) {
  throw new Error(`JSONPath extended mismatch: expected=${JSON.stringify(expected)} actual=${JSON.stringify(actual)}`);
}

console.log(JSON.stringify({
  status: 'passed',
  contract: 'legado_jsonpath_runtime_extended_execution',
  assertions: 6,
  result: actual,
