param(
  ...
  [string]$ResultPath = 'tools/legado-compat/evidence/v2-jsoup-attribute-and-tag-token-current-head-audit-20260810.json'
)
...
function Assert-Audit { param([bool]$Condition, [string]$Id, [string]$Detail, [string[]]$Evidence = @()); if (-not $Condition) { throw "243 attribute/tag token current-head audit failed: $Detail" }; $script:assertions++; [void]$script:checks.Add([pscustomobject][ordered]@{ id = $Id; status = 'passed'; detail = $Detail; evidencePaths = @($Evidence) }) }
...
Assert-Audit ([string]$fixture.issueId -eq $issueId -and @($fixture.cases).Count -eq 3) 'fixture' 'all token-context cases remain bound to 243.' @($FixturePath)
...
Assert-Audit ($analyzer.Contains('const scan = this.scanNextHtmlTag(content, cursor);') -and $analyzer.Contains('cursor = scan.endIndex + 1;')) 'tag-scanner' 'tag lookup is scanner-backed.' @($analyzerPath)
Assert-Audit ($analyzer.Contains('searchStart = startTagEnd + 1;') -and -not $analyzer.Contains('searchStart = tagStart + 1;')) 'caller-boundary' 'all tag/text selector callers resume outside the start tag.' @($analyzerPath)
Assert-Audit ($analyzer.Contains("this.extractAttributeValueFromTagContent(tagContent, 'class')") -and $analyzer.Contains("this.extractAttributeValueFromTagContent(tagContent, 'id')")) 'attribute-consumers' 'class and id lookup consume parsed start-tag attributes.' @($analyzerPath)
Assert-Audit ($analyzer.Contains('while (cursor < sourceLength && source[cursor] !== quote)') -and $analyzer.Contains('if (name === lowerAttrName)')) 'attribute-parser' 'quoted attribute values are consumed atomically and compared by parsed name.' @($analyzerPath)
Assert-Audit (-not $analyzer.Contains('this.findClassAttrIndex(') -and -not $analyzer.Contains('this.findIdAttrIndex(') -and -not $analyzer.Contains('content.indexOf(''<'' , cursor)')) 'old-path-closure' 'quote-blind token discovery calls are absent.' @($analyzerPath)
...
$result = [pscustomobject][ordered]@{
  ...
  verificationPolicy = 'r3_243_attribute_and_tag_token_current_head_static_only;runtime_build_device_and_legado_diff_deferred_to_R4'
  nextGate = 'Register the scanner-backed tag and attribute source fix and keep 243 verifying until R4 runtime and Legado differential gates.'
}
