...
param(
  ...
  [string]$OutputPath = 'tools/legado-compat/evidence/v2-arkweb-jsoup-empty-compound-current-head-audit-20260810.json'
)
...
function Assert-Audit { param([bool]$Condition, [string]$Message); if (-not $Condition) { throw "ArkWeb 243 empty compound current-head audit failed: $Message" }; $script:assertions++ }
...
Assert-Audit ([string]$fixture.contract -eq 'arkweb_legado_jsoup_empty_compound_pseudo_context' -and @($fixture.cases).Count -eq 6) 'empty-compound fixture drifted.'
...
Assert-Audit ($runtime.Contains('var targetSelector = legadoSelectorCompoundBefore(normalized, normalized.length);') -and $runtime.Contains("if (!targetSelector) {") -and $runtime.Contains("targetSelector = '*';") -and $runtime.Contains("normalized += '*';")) 'current head lacks the universal repair for pseudo-only compounds.'
Assert-Audit ($runtime.Contains("targetScope: isStandardDocumentPseudo ? 'first-compound' : (isManualPseudo ? 'compound' : '')") -and $runtime.Contains('targetSelector: targetSelector')) 'current head does not retain the repaired owning-compound metadata.'
...
$result = [pscustomobject][ordered]@{
  ...
  verificationPolicy = 'r3_243_arkweb_empty_compound_current_head_static_only;runtime_build_device_and_legado_diff_deferred_to_R4'
}
