...
param(
  ...
  [string]$ResultPath = 'tools/legado-compat/evidence/v2-jsoup-regex-class-parenthesis-current-head-audit-20260810.json'
)
...
function Assert-Audit { param([bool]$Condition, [string]$Message); if (-not $Condition) { throw "243 regex-class parenthesis current-head audit failed: $Message" }; $script:assertions++ }
...
Assert-Audit ($chainSplitStart -gt $chainStart -and $chainSplitEnd -gt $chainSplitStart -and $selectorSplitStart -ge 0 -and $selectorSplitEnd -gt $selectorSplitStart -and $groupSplitStart -ge 0 -and $groupSplitEnd -gt $groupSplitStart) 'splitter boundaries are missing.'
...
Assert-Audit ($chainBody.Contains("if (c === '(' && bracketDepth === 0)") -and $selectorBody.Contains("if (character === '(' && bracketDepth === 0)") -and $groupBody.Contains("if (character === '(' && bracketDepth === 0)")) 'bracket-opaque open-parenthesis guards are missing.'
Assert-Audit ($chainBody.Contains("bracketDepth === 0)" ) -and $selectorBody.Contains("bracketDepth === 0)" ) -and $groupBody.Contains("bracketDepth === 0)" )) 'bracket-opaque close/dispatch guards are missing.'
...
$result = [pscustomobject][ordered]@{
  ...
  verificationPolicy = 'r3_243_regex_class_parenthesis_current_head_static_only;runtime_build_device_and_legado_diff_deferred_to_R4'
  nextGate = 'Register the static source fix and keep 243 verifying until R4 runtime and Legado differential gates.'
}
