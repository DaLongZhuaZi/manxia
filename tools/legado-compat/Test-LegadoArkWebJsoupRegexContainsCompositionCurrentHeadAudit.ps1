...
param(
  ...
  [string]$OutputPath = 'tools/legado-compat/evidence/v2-arkweb-jsoup-regex-contains-composition-current-head-audit-20260810.json'
)
...
function Assert-Audit { param([bool]$Condition, [string]$Message); if (-not $Condition) { throw "ArkWeb 243 regex/contains current-head audit failed: $Message" }; $script:assertions++ }
...
Assert-Audit ([string]$fixture.contract -eq 'arkweb_legado_jsoup_regex_contains_composition_context' -and @($fixture.affectedSourceOrdinals).Count -eq 1 -and [int]$fixture.affectedSourceOrdinals[0] -eq 267) 'ordinal-267 fixture drifted.'
...
Assert-Audit ($runtime.Contains('legadoMatchesJsoupSelector(ancestor, pseudo.targetSelector, documentRoot)') -and -not $runtime.Contains('if (ancestor.matches && ancestor.matches(pseudo.targetSelector))')) 'current head must use the Jsoup-aware owning-compound bridge.'
...
$result = [pscustomobject][ordered]@{
  ...
  verificationPolicy = 'r3_243_arkweb_regex_contains_composition_current_head_static_only;runtime_build_device_and_legado_diff_deferred_to_R4'
}
