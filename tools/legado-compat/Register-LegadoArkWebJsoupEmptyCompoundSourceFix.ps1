...
param(
  ...
  [string]$PostFixContractPath = 'tools/legado-compat/evidence/contract-legado-arkweb-jsoup-empty-compound-post-fix-20260810.json',
  [string]$CurrentHeadAuditPath = 'tools/legado-compat/evidence/v2-arkweb-jsoup-empty-compound-current-head-audit-20260810.json',
  [string]$SourceFixPath = 'tools/legado-compat/evidence/v2-arkweb-jsoup-empty-compound-source-fix-20260810.json'
)
...
$backupPath = 'entry/src/main/resources/rawfile/legado_runtime.html.bak_20260810_issue243_empty_compound'
...
function Assert-Gate { param([bool]$Condition, [string]$Message); if (-not $Condition) { throw "ArkWeb 243 empty compound source-fix gate failed: $Message" } }
...
Assert-Gate ($backup.Contains('var targetSelector = legadoSelectorCompoundBefore(normalized, normalized.length);') -and -not $backup.Contains("if (!targetSelector) {") -and -not $backup.Contains("normalized += '*';")) 'the pre-fix backup does not capture the missing universal compound repair.'
Assert-Gate ($runtime.Contains("if (!targetSelector) {") -and $runtime.Contains("targetSelector = '*';") -and $runtime.Contains("normalized += '*';")) 'current ArkWeb source does not retain the universal compound repair.'
...
$sourceFix = [pscustomobject][ordered]@{
  ...
  extends = 'tools/legado-compat/evidence/v2-arkweb-jsoup-not-argument-source-fix-20260810.json'
  ...
  rootCause = [pscustomobject][ordered]@{
    category = '规则解析或编译'
    originalSemantics = 'Pinned Legado delegates CSS selection to Jsoup, where a pseudo-only compound remains an element compound and therefore behaves as the universal selector.'
    v2BeforeFix = 'ArkWeb removed the manual pseudo token from the selector text without preserving an element token, leaving invalid leading or trailing combinators such as >a, head> and p>; consecutive or sibling-joined pseudos in empty compounds widened the same defect.'
    v2AfterFix = 'The parser records every empty owning compound as * and inserts * into the normalized selector before browser selection, while retaining pseudo targets for predicate evaluation across left, right, repeated and sibling-joined compounds.'
  ...
  verificationPolicy = 'r3_243_arkweb_empty_compound_source_fix_static_only;R4_runtime_build_device_and_legado_diff_deferred'
...
Set-PropertyValue $objective 'targetRevision' '2026-08-10-actual-docs-source-refactor-arkweb-jsoup-empty-compound-static-closure'
Set-PropertyValue $objective 'continuationMode' 'R3_ISSUE_243_ARKWEB_EMPTY_COMPOUND_STATIC_CLOSED_WAIT_R4'
Set-PropertyValue $objective.authority 'activeIssueId' $issueId
Set-PropertyValue $objective.authority 'activeIssueSelection' 'full-source-validation-state.json remains the only queue; 243 remains the sole active source-closure issue. ArkWeb pseudo-only compound repair is statically closed; semanticMatchAllowed remains false and R4 is deferred.'
Set-PropertyValue $objective.objective 'activeIssueRule' '243 covers owning compounds and selector-root preservation: every manually evaluated pseudo must retain an element token for its compound, including pseudo-only operands after direct-child and descendant combinators.'
Set-PropertyValue $objective 'nextAction' '243 ArkWeb pseudo-only compound source closure is registered; continue one-root-cause source audit while R4 remains deferred.'
if ($null -ne $objective.PSObject.Properties['continuationTarget']) {
  Set-PropertyValue $objective.continuationTarget 'activeBoundary' '243 remains verifying; ArkWeb pseudo-only compound context, ordinals 357 and 402, failure witness, post-fix and current-head evidence are registered; R4 remains deferred.'
  Set-PropertyValue $objective.continuationTarget 'nextTransition' 'Continue auditing selector-root preservation for repeated and nested pseudo-only compounds; keep runtime and differential gates deferred.'
}
$plan = @($objective.continuationPlan)
if (-not @($plan | Where-Object { [string]$_.id -eq '243-SP-59' })) {
  $plan += [pscustomobject][ordered]@{ id = '243-SP-59'; status = 'completed'; action = '修复 ArkWeb 手工伪类独占 compound 时移除伪类留下非法 >a、head>、p> 选择器的问题，补回通用 *，覆盖左侧、右侧、连续及 sibling 连接形态并登记第 357、402 条书源规则。'; evidence = @($FixturePath, $FailureWitnessPath, $PostFixContractPath, $CurrentHeadAuditPath, $SourceFixPath) }
  $plan += [pscustomobject][ordered]@{ id = '243-SP-60'; status = 'deferred'; action = 'R4 执行六个空 compound 案例、第 357/402 条书源、458 条 Harness、Legado 差分、构建和真机验证。' }
} else {
  $completed = @($plan | Where-Object { [string]$_.id -eq '243-SP-59' })[0]
  Set-PropertyValue $completed 'action' '修复 ArkWeb 手工伪类独占 compound 时移除伪类留下非法 >a、head>、p> 选择器的问题，补回通用 *，覆盖左侧、右侧、连续及 sibling 连接形态并登记第 357、402 条书源规则。'
  ...
  $deferred = @($plan | Where-Object { [string]$_.id -eq '243-SP-60' })[0]
  if ($null -ne $deferred) { Set-PropertyValue $deferred 'action' 'R4 执行六个空 compound 案例、第 357/402 条书源、458 条 Harness、Legado 差分、构建和真机验证。' }
}
Set-PropertyValue $objective 'continuationPlan' $plan
Write-AtomicJson $statePath $state
Write-AtomicJson $objectivePath $objective

$evidence = @($FixturePath, $FailureWitnessPath, $PostFixContractPath, $CurrentHeadAuditPath, $SourceFixPath, $runtimePath, $backupPath, 'tools/legado-compat/Test-LegadoArkWebJsoupEmptyCompoundFailureWitness.ps1', 'tools/legado-compat/Test-LegadoArkWebJsoupEmptyCompoundPostFixContract.ps1', 'tools/legado-compat/Test-LegadoArkWebJsoupEmptyCompoundCurrentHeadAudit.ps1', 'tools/legado-compat/Register-LegadoArkWebJsoupEmptyCompoundSourceFix.ps1', $legadoPath)
$summary = '243 ArkWeb pseudo-only compound closure: manual pseudos now preserve universal element tokens for left, right, repeated and sibling-joined empty compounds. Ordinals 357 and 402 are bound to static failure, post-fix and current-head evidence. Runtime, full Harness, Legado differential, build and device validation remain deferred.'
